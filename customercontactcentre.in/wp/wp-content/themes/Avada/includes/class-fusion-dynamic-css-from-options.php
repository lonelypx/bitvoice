...
class Fusion_Dynamic_CSS_From_Options {
	...
	private function init( $context = 'TO' ) {
		...
		if ( 'PO' === $context ) {
			...
			$current_page_id = Avada()->fusion_library->get_page_id();

			// Get the post-meta.
			$this->post_meta = get_post_custom( $current_page_id );
	...
	private function generate_css_from_field( $field, $context = 'TO' ) {
		...
		// Get the value.
		if ( 'PO' === $context ) {
			$value = ( isset( $this->post_meta[ $id ] ) ) ? $this->post_meta[ $id ] : null;
