...
final class Avada_Google_Fonts {
	...
	public function the_local_fonts_css() {
		$css = '';
		if ( isset( $_GET['action'] ) && 'get-gfonts' === sanitize_text_field( wp_unslash( $_GET['action'] ) ) && isset( $_GET['family'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification

			header( 'Content-type: text/css; charset: UTF-8' );

			// Split font-families.
			$families = explode( '|', sanitize_text_field( wp_unslash( $_GET['family'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification
