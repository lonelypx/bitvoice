	...
	public function adopt_to_builder() {
		...
		if ( $this->is_fb_enabled( $post_type ) && is_object( $post ) ) {
			if ( isset( $_GET['fb-be-editor'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				$button       = '<a href="' . $post_link . '" id="fusion_builder_switch" class="button button-primary button-large"><span class="fusion-builder-button-text">' . esc_html( $editor_label ) . '</span></a>';
				?>
				<script type="text/javascript">
				jQuery( window ).load( function() {
	...
	public function add_edit_link( $actions, $post ) {
		...
		$edit_action = [
			'fusion_builder_live' => sprintf(
				...
				__( 'Fusion Builder Live', 'fusion-builder' )
			),
			'gutenberg'           => sprintf(
				'<a href="%s" aria-label="%s">%s</a>',
