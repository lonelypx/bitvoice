			...
			validateValues: function( atts ) {
				...
				if ( 'undefined' !== typeof values.image && ( '' !== values.image || '' !== values.image_id ) ) {
					...
					if ( imageData && '' !== imageData.height ) {
						values.image_width  = -1 === values.image_width || '' === values.image_width ? imageData.width : values.image_width;
						values.image_height = imageData.width ? ( parseFloat( values.image_width ) / parseFloat( imageData.width ) ) * parseFloat( imageData.height ) : values.image_width;
					} else {
						values.image_width  = '35';
						values.image_height = '35';
					}
				} else {
					values.image_width  = -1 === values.image_width || '' === values.image_width ? '35' : values.image_width;
					values.image_height = '35';
				}
